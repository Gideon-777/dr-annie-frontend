...
class Ct extends React.Component {
    ...
    async loadComponent() {
        try {
            this.model = await tf.loadLayersModel('https://covid19-241.herokuapp.com/CTmodel/model.json');
        } catch (e) {
            alert('Impossible de charger le modèle de prédiction')
        }
    }
    ...
    async predict(image) {
        try {
            ...
            const mapper = ["CT_COVID","CT_NonCOVID"]
    ...
    render() {
        return (
            ...
            <div>
              <strong>
                <h1 style={{ color: '#000000' }}>VEUILLEZ SÉLECTIONNER LES IMAGES À DIAGNOSTIQUER (SCAN DES POUMONS)</h1>
              </strong>

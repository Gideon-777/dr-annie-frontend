...
class DisplayFetchedLocation extends React.Component {
  state = {
    info: null,
    loading: true,
    nextStep: null
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (!this.state.loading && prevState.loading) {
      setTimeout(() => this.props.triggerNextStep({
        value: this.props.steps['11'].value,
  ...
  componentDidMount() {
    axios.get(`/api/covid-data/countries/${this.props.steps['11'].value}`).then(res => {
      const info = res.data;
      console.log(info);
      if (info.data.count === 0) {
        return {nextStep: '11', message: 'Impossible de trouver ce pays. Essayer le nom du pays en anglais'};
      } else if (info.data.count > 1) {
        return {nextStep: 'fetchFetchedCountryStateName', message: 'Please enter the name of your state.'}
      } else {
...
export default DisplayFetchedLocation;
